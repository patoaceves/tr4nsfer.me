...
// Rate limited: 20 checks per IP per minute.
// Hashes the email with SHA-256 before querying — never stores raw email in this check.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!email) return new Response(JSON.stringify({ error: 'email requerido' }), { status: 400, headers: CORS })

    const emailClean = String(email).toLowerCase().trim()
    if (!EMAIL_RE.test(emailClean)) {
      return new Response(JSON.stringify({ error: 'formato de email inválido' }), { status: 400, headers: CORS })
    }
    ...
    const allowed = await checkRateLimit(supabase, `email-check:${ip}`, 20, 60 * 1000)
    if (!allowed) {
      return new Response(JSON.stringify({ error: 'Demasiadas solicitudes. Intenta en un minuto.' }), { status: 429, headers: CORS })
    }

    const hash = await hashEmail(emailClean)
    const { data } = await supabase.from('links').select('id').eq('email_hash', hash).maybeSingle()
